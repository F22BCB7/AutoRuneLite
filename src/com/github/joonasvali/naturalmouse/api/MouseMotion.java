...
public class MouseMotion {
  ...
  public void move(MouseMotionObserver observer) throws InterruptedException {
    updateMouseInfo();
    //System.out.println("Starting to move mouse to ({"+xDest+"}, {"+yDest+"}), current position: ({"+mousePosition.x+"}, {"+mousePosition.y+"})");
    ...
    while (mousePosition.x != xDest || mousePosition.y != yDest) {
      if (movements.isEmpty()) {
        ...
        // to wrong pixel)
        updateMouseInfo();
        //System.out.println("Re-populating movement array. Did not end up on target pixel.");
        movements = movementFactory.createMovements(mousePosition);
      }

      Movement movement = movements.removeFirst();
      if (!movements.isEmpty()) {
    	  //System.out.println("Using overshoots ({"+(overshoots - movements.size() + 1)+"} out of {"+overshoots+"}), aiming at ({"+movement.destX+"}, {"+movement.destY+"})");
      }
      ...
      //System.out.println("Movement arc length computed to {"+distance+"} and time predicted to {"+mouseMovementMs+"} ms");
      ...
      for (int i = 0; i < steps; i++) {
        ...
        //System.out.println("Step: x: {"+xStepSize+"} y: {"+yStepSize+"} tc: {"+timeCompletion+"} c: {"+completion+"}");
        ...
        //System.out.println("EffectFadeMultiplier: {"+effectFadeMultiplier+"}");
        //System.out.println("SimulatedMouse: [{"+simulatedMouseX+"}, {"+simulatedMouseY+"}]");
      ...
      if (mousePosition.x != movement.destX || mousePosition.y != movement.destY) {
        ...
        // But print warning as this is not expected behavior.
    	  /*System.out.println("Mouse off from step endpoint (adjustment was done) " +
      ...
      //System.out.println("Steps completed, mouse at " + mousePosition.x + " " + mousePosition.y);
    }
    //System.out.println("Mouse movement to ({"+xDest+"}, {"+yDest+"}) completed");
  }
  ...
  private void sleepAround(long sleepMin, long randomPart) throws InterruptedException {
    ...
    if (sleepTime > 0) {
      updateMouseInfo();
      //System.out.println("Sleeping at ({"+mousePosition.x+"}, {"+mousePosition.y+"}) for {"+sleepTime+"} ms");
    }
